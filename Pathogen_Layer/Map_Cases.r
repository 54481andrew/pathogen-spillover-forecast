...
## Load rodent dataset
classi.dat.rod <- read.csv(file = 'Data/Prepped_Pathogen_PresAbs_Data.csv')
human.test.dat <- read.csv(file = 'Data/Prepped_Human_Seroprevalence_Data.csv')
...
classi.dat$BiStatus <- factor(classi.dat$BiStatus,
                              levels = c('Rodent, Lassa +', 'Rodent, Lassa -', 'Human, Arenavirus +', 'Human, Arenavirus -'),
                              ordered = TRUE)
...
g3 <- g1 +
      annotation_custom(grob = g2, xmin = 5, xmax = 15,
                        ymin = 17, ymax = 27)
g3
ggsave(filename = paste('Figures_Fits/Data_Map.png', sep=''), device = 'png',
